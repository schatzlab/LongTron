...
$BEDTOOLS intersect -sorted -a pre_novel_junctions.split_ends.${WIGGLE}.bed -b <(zcat ../../gtex_sra_junctions.split.bed.bgz) -v | perl -ne 'BEGIN { %h; } chomp; $f=$_; ($c,$c1,$c2,$o,$nr,$reads,$pac,$nlr,$dist,$coord,$t,$junctions)=split(/\t/,$f); $tnot=!$t; $k=$coord."-".$tnot; if($h{$k}) { print "".$h{$k}."\n"; delete($h{$k}); next; } $k=$coord."-".$t; $coord=~/^(\d+)-(\d+)$/; $c1=$1; $c2=$2; $h{$k}=join("\t",($c,$c1,$c2,$o,$nr,$reads,$pac,$nlr,$dist,$junctions));' > novel_junctions_bedtools.wiggle.${WIGGLE}
