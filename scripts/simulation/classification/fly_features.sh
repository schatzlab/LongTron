...
GENOME_INDEX=dmel-all-chromosome-r6.23_.fasta
GENOME_SIZES=R623.fly.chrom.sizes
...
LOCUS_STATS=$SOURCE_PATH/locus.stats.bed.nomt
...
$BT closest -g R623.fly.chrom.sizes.sorted -s -t first -D ref -a ${IN}.ot.ed.td.logsX3.sm.gc -b ${LOCUS_STATS} |  perl -ne 'chomp; $f=$_; @f=split(/\t/,$f); @f1=splice(@f,0,('$OFFSET'+9)); $all=join("\t",@f1); @f2=splice(@f,6); $all2=join("\t",@f2); print "$all\t".$f[4]."\t$all2\n";' > ${IN}.ot.ed.td.logsX3.sm.gc.lsX10
