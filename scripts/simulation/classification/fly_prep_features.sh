...
ANNOT_VER=R623
CHROM_SIZES=${ANNOT_VER}.fly.chrom.sizes
NUM_CHRMS=25
ANNOTATION=dmel-all-r6.23.gtf_.gz
GENOME=dmel-all-chromosome-r6.23_.fasta
RM=rmsk.txt.gz
GC=gc5Base.bw
...
#get chrm sizes
cat $GENOME | perl -ne 'chomp; $s=$_; if($s=~/^>/) { if($seq && length($seq) > 0) { print "\t".length($seq)."\n"; } $seq=""; $s=~s/^>([^\s]+).*$/$1/; print "$s"; next; } $seq.=$s; END { if($seq && length($seq) > 0) { print "\t".length($seq)."\n"; } }' > $CHROM_SIZES
...
zcat $ANNOTATION | egrep -e '	(\w+RNA)|(pseudogene)	' | cut -f 1,4,5,7 | sort -k1,1 -k2,2n -k3,3n | perl -ne 'BEGIN { open(OUT,">'${ANNOTATION}'.transcripts.sorted.bed"); } chomp; $f=$_; ($c,$s,$e,$o)=split(/\t/,$f); print OUT "$c\t".($s-1)."\t$e\n"; if(!$pc || ($pc ne $c || $s > $pe)) { if($pc) { $ps--; for $i ($ps..($pe-1)) { print "$pc\t$i\t".($i+1)."\n"; }} $ps=$s; $pc=$c; $pe=$e; $po=$o; next; } $pe=$e; END { if($pc) { $ps--; for $i ($ps..($pe-1)) { print "$pc\t$i\t".($i+1)."\n"; } } close(OUT);}' | bgzip > ${ANNOTATION}.transcripts.perbase.bgz
