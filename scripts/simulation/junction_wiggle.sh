...
$BT window -a ${JUNCTIONS}.clean.bed -b ${ANNOT_JUNCTIONS}.bed -w $WIGGLE | perl -ne 'BEGIN { $w='${WIGGLE}'; open(IN,"<'${TID_FILE}'"); %tids; while($line=<IN>) { chomp($line); ($tid,$n)=split(/\t/,$line); $n=~s/_0_[+-]$//; $tids{$tid}=$n; } close(IN); %mtids; } chomp; $f=$_; ($c1,$s1,$e1,$o,$nr,$reads,$c2,$s2,$e2,$t,$eid)=split(/\t/,$f); $t=~s/_0_[+-]$//; $d1=abs($s1-$s2); $d2=abs($e1-$e2); $c3=($d1>$d2?$s1:$e1); $k="$c1\t$s1\t$e1"; $matched_tid=0; for $tid (split(/,/,$reads)) { ($tid,$eid2)=split(/\./,$tid); if($tids{$tid} eq $t && $eid eq $eid2 && $mtids{$k} != 2) { $matched_tid=1; $mtids{$k}=1; break; } }  $line=join("\t",($c1,$s1,$e1,$o,$nr,$reads,$c3)); if($d1 <= $w && $d2 <= $w) { $mtids{$k} = 2 if($matched_tid == 1); delete $h2{$k}; $h{$k}=$line; } else { if(!$h{$k}) { $h2{$k}=$line; } }  END { for $k (keys %h2) { $matched=$mtids{$k}; $matched=0 if(!$mtids{$k}); $k=$h2{$k}; print "$k\t$matched\n"; } for $k (keys %h) { $matched=$mtids{$k}; $matched=0 if(!$mtids{$k}); $k=$h{$k}; print STDERR "$k\t$matched\n"; }}' 2> sim10.junctions.clean.w${WIGGLE}.matching > sim10.junctions.clean.w${WIGGLE}.nonmatching
...
cat sim10.junctions.split_ends.bed.w${WIGGLE}.sorted | perl -ne 'BEGIN { %h; } chomp; $f=$_; ($c,$c1,$c2,$o,$nr,$reads,$coord)=split(/\t/,$f); $k=$coord; if($h{$k}) { print "".$h{$k}."\n"; delete($h{$k}); next; } $k=$coord; $coord=~/^(\d+)-(\d+)$/; $c1=$1; $c2=$2; $h{$k}=join("\t",($c,$c1,$c2,$o,$nr,$reads));' | sort -k5,5nr -k1,1 -k2,2n -k3,3n > sim10.junctions.split_ends.bed.w${WIGGLE}.both_ends
...
cat sim10.junctions.split_ends.bed.w${WIGGLE}.sorted | perl -ne 'BEGIN { %h; } chomp; $f=$_; ($c,$c1,$c2,$o,$nr,$reads,$coord)=split(/\t/,$f); $k=$coord; $coord=~/^(\d+)-(\d+)$/; $c3=$c1; $c1=$1; $c2=$2; $d1=abs($c1-$c3); $d2=abs($c2-$c3); $c3=($d1>$d2?$c2:$c1); if(!$h{$k}) { print "".join("\t",($c,--$c1,$c2,$o,$nr,$reads,$c3))."\n"; } $h{$k}=1;' | sort -k5,5nr -k1,1 -k2,2n -k3,3n > sim10.junctions.split_ends.bed.w${WIGGLE}.either_end
...
#sort -k1,1 -k2,2n -k3,3n sim10.junctions.split_ends.bed.w${WIGGLE}.either_end > $ALL_NONMATCH
