...
//this is a slightly modified version of Heng Li's original script:
//https://github.com/lh3/minimap2/blob/master/misc/paftools.js
//
//the junceval method was modified to:
//1) take comparison jx read support into account when doing fuzzy matching
//2) output a different format that reports the read support for the comparison jxs ("annotation")
//3) some other output related tweaks
...
function paf_junceval(args)
{
	...
	if (args.length - getopt.ind < 1) {
		...
		print("  -e        dont print erroneous overlapping introns");
	...
	while (file.readline(buf) >= 0) {
		...
		var tid = m[1];
		if (tr[tid] == null) tr[tid] = [t[0], t[6], 0, 0, [], 0];
		tr[tid][4].push([st, en]);
		tr[tid][5] = t[5];
	}
	file.close();
		...
		for (var i = 0; i < s.length - 1; ++i) {
			if (s[i][1] >= s[i+1][0])
				warn("WARNING: incorrect annotation for transcript "+tid+" ("+s[i][1]+" >= "+s[i+1][0]+")")
					anno[t[0]].push([s[i][1], s[i+1][0], t[5]]);
		}
	}
	tr = null;
			...
			for (var i = 0; i < intron.length; ++i) {
				...
				if (o.length > 0) {
					...
					for (var j = 0; j < o.length; ++j) {
						...
						//if exact match, we take the first one
						if (intron[i][0] == o[j][0] && intron[i][1] == o[j][1])
						{
							hit = true;
							single_hit = o[j];
							break;
						}
						//if fuzzy match we'll keep looking for the best annotated exon match
						if (st_diff <= l_fuzzy && en_diff <= l_fuzzy && single_hit[2] < o[j][2])
							//++n_splice_hit, hit = true, single_hit = o[j];
							hit = true, single_hit = o[j];
						//if (hit) break;
					}
					if (print_ovlp) {
						...
						//only print the chr:start-end of the read intron
						print(t[2], intron[i][0], intron[i][1], single_hit[0], single_hit[1], single_hit[2], t[0], i+1);
					}
					if (hit)
						++n_splice_hit;
				} else {
...
// if (!print_ovlp) {
		warn("# unmapped reads: " + n_unmapped);
		warn("# mapped reads: " + n_mapped);
		warn("# primary alignments: " + n_pri);
		warn("# singletons: " + n_sgl);
		warn("# predicted introns: " + n_splice);
		warn("# non-overlapping introns: " + n_splice_novel);
		warn("# correct introns: " + n_splice_hit + " (" + (n_splice_hit / n_splice * 100).toFixed(2) + "%)");
//	}
}
