...
gtf1=$2
gtf2=$3
#e.g. amount of wiggle around splice site matches, 0=exact, 20 is the other option we used
fuzz=$4
...
ln -fs ../${gtf1}
ln -fs ../${gtf2}
../gcom -r $gtf1 --fuzz-length $fuzz $gtf2 > run 2>&1
...
if [[ "$FLIP_STRAND" == "1" ]]; then
    ...
    ../gcom -r $gtf1 --fuzz-length $fuzz ${gtf2}.o_s_strand_flipped > run 2>&1
